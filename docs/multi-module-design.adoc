...
* **Version Inheritance**: Child modules inherit version from aggregator; `<version>` is optional in child modules
...
* Can be root or nested within another aggregator (nested aggregators supported)
...
**Note:** Library projects do not require `<mainSource>`. PasBuild automatically generates a bootstrap program that discovers and compiles all units in the configured unit paths. The `<version>` element is optional in child modules -- it is inherited from the aggregator.
...
Algorithm:
1. Load root aggregator project.xml
2. Validate packaging = pom
3. Seed visited-aggregator set with root path
4. Recursively discover modules (DiscoverModulesRecursive):
   For each <module> path in current aggregator's <modules>:
   a. Resolve absolute path relative to current aggregator directory
   b. Check visited-aggregator set for circular references
   c. Validate path is within root project tree
   d. Validate project.xml exists at path
   e. Load module's project.xml (allow empty version for inheritance)
   f. Version inheritance: if module has no version, inherit from parent aggregator
   g. Create TModuleInfo with name, path, config
   h. Register in shared TModuleRegistry
   i. If module is packaging=pom with <modules>:
      - Add path to visited-aggregator set
      - Recurse into module's children (arbitrary depth)
5. Second pass — for each registered module:
   a. Process <moduleDependencies> section
   b. For each dependency path:
      - Resolve absolute path relative to module's directory
      - Validate path within root project tree
      - Find module in registry by path
      - Add module name to TModuleInfo.Dependencies
6. Return flat TModuleRegistry (suitable for topological sort)
----
...
**Resolution: Version Inheritance (implemented)**

Child modules inherit their version from the aggregator. The `<version>` element is optional in child module `project.xml` files:

* If `<version>` is omitted, the aggregator's version is inherited automatically
* If `<version>` is present and matches the aggregator, it is accepted (backward compatible)
* If `<version>` is present but differs from the aggregator, a build error is raised with guidance to either remove the element or correct it

This follows the Maven model where child POMs inherit `<version>` from their parent, adapted for PasBuild's simpler structure (no `<parent>` element needed since the aggregator already knows its children via `<modules>`).

The recommended practice is to define `<version>` only in the root aggregator `project.xml` and omit it from all child modules.
...
**Resolution: Nested aggregators supported (implemented)**

Option A was implemented. `TModuleDiscoverer.DiscoverModules` performs recursive discovery to arbitrary depth. When a discovered module has `packaging=pom` with its own `<modules>` list, the discoverer recurses into its children, collecting all modules into a single flat `TModuleRegistry`.

Key implementation details:

* Cycle detection via visited aggregator path tracking (normalised with `ExcludeTrailingPathDelimiter`)
* Version inheritance chains through all nesting levels (root → mid-aggregator → leaf)
* Path validation always checks against the root aggregator directory
* The reactor and topological sort operate on the flat module list unchanged
* POM-type modules in the registry are skipped during build execution (existing behaviour)

This enables organising large projects with nested groups (e.g., `examples/` as a sub-aggregator containing many small demo applications) without listing every leaf module in the root aggregator.
