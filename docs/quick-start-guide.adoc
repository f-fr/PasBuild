...
* **Nested aggregators** - Group related modules under sub-aggregators (e.g., `examples/`)
...
Nested aggregators can be nested to arbitrary depth. Module discovery recursively walks the aggregator tree and produces a single flat module list for the reactor. Version inheritance flows through the full aggregator chain: root → sub-aggregator → leaf module.
...
pasbuild dependency-tree -m demo1  # Show only demo1's dependencies
----
...
**For detailed dependency management documentation**, see link:dependency-management-design.adoc[Dependency Management Design].
...
For the full plugin contract, example implementations (shell script and Pascal), and
...
**Dependencies:** Automatically runs `process-resources` -> `compile` -> `install`
...
pasbuild resolve -m fpgui-framework   # Specific module in a multi-module project
----
...
* `compiler` — FPC executable path and the full command line that `compile` would invoke
* `defines` — all active preprocessor defines (global + profile)
* `unitPaths` — resolved source directories passed to FPC as `-Fu` flags
* `includePaths` — resolved include directories passed to FPC as `-Fi` flags
...
_which directories_ are active for the current configuration; scanning for specific files
...
* Plugin development — call `pasbuild resolve` from a plugin to obtain full project context
  without hardcoding any paths
* Debugging — inspect exactly which paths and defines PasBuild has resolved before running
  a build
...
* `pascal` — source files are in a `pascal/` subdirectory
* `src` — source files are in a `src/` subdirectory
* _(omitted)_ — defaults to `src/main/pascal/`
...
* Write a plugin — see `extras/plugins/hello-world/README.adoc` for the plugin contract
