...
External dependencies are resolved from `~/.pasbuild/repository/` at compile time. Use `pasbuild install` in the dependency project to make it available. See link:dependency-management-design.adoc[Dependency Management Design] for details.
...
=== Example: Framework with Nested Examples
...
=== Leaf Modules with Cross-Level Dependencies
...
=== Custom Source Directory for Examples
